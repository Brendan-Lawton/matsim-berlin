...
public class Emission {
	...
	public static void main(String[] args) {
		...
		config.vehicles().setVehiclesFile("/Users/korsun/Downloads/berlin-v6-2.3-10pct_policy/berlin-v6.3.output_vehicles.xml.gz");//change directory
		config.network().setInputFile("/Users/korsun/Desktop/MATSIM/Script_add_type/modified_network3.xml");//change directory
		config.transit().setTransitScheduleFile("/Users/korsun/Downloads/berlin-v6-2.3-10pct_policy/berlin-v6.3.output_transitSchedule.xml.gz");//change directory
		config.transit().setVehiclesFile("/Users/korsun/Downloads/berlin-v6.3-10pct_policy/berlin-v6.3.output_transitVehicles.xml.gz");//change directory
		...
		for (Link link : scenario.getNetwork().getLinks().values()) {
			// if link is not pt or just bike
			if (!link.getAllowedModes().contains("pt") &&
				!(link.getAllowedModes().size() == 1 && link.getAllowedModes().contains("bike"))) {


				String currentType = NetworkUtils.getType(link);
				if (currentType != null) {

					String newType = currentType.replaceFirst("highway.", "");
					NetworkUtils.setType(link, newType);
				} else {
					// if no link type
					System.out.println("Warning: Link ID " + link.getId() + " has no type attribute.");
				}
			}
		}
		...
		var writer = new EventWriterXML("/Users/korsun/Desktop/MATSIM/emission_policy.xml.gz");//change directory
		manager.initProcessing();
		manager.addHandler(writer);
		EventsUtils.readEvents(manager, "/Users/korsun/Downloads/berlin-v6-2.3-10pct_policy/berlin-v6.3.output_events.xml.gz");//change directory
